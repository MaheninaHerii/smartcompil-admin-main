...
import {
    deleteIntroductionNousRejoindreRequest, fetchIntroductionNousRejoindreListRequest,
...
import {
    ...
    getIntroductionNousRejoindreListSelector,
...
export default function IntroductionNousRejoindreList() {
    ...
    const rows = useSelector(getIntroductionNousRejoindreListSelector);
    ...
    useEffect(() => {
        dispatch(fetchIntroductionNousRejoindreListRequest());
    }, [dispatch]);
    ...
    const handlePublished = () => {
        dispatch(putUpdateStateIntroductionNousRejoindreRequest(currentRow));
        handleClose();
    }

    const handleDelete = () => {
        dispatch(deleteIntroductionNousRejoindreRequest(currentRow && currentRow.id || 0));
        handleCloseConfirmDialog();
    }

    const goToDetail = () => {
        history(`/introduction-nous-rejoindre/detail/${currentRow.id}`);
    }

    const goToEdit = () => {
        history(`/introduction-nous-rejoindre/modifier/${currentRow.id}`);
    }

    const goToNew = () => {
        history(`/introduction-nous-rejoindre/nouveau`);
    }
